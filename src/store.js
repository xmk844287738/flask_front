export default {
  ...
  state: {
    ...
    user_name: window.localStorage.getItem('blog-token') ? JSON.parse(atob(window.localStorage.getItem('blog-token').split('.')[1])).user_name : '',
    // 用户登录后，就算刷新页面也能再次计算出 user_avatar
    // 后端传 URL 必须先用 base64 编码，所以这里还要多进行一次 atob 解码 base64 字符串
    user_avatar: window.localStorage.getItem('blog-token') ? atob(JSON.parse(atob(window.localStorage.getItem('blog-token').split('.')[1])).user_avatar) : ''
  },
  loginAction () {
